	...
	_this.fileStream.on('data', function(data) {
		...
		while((itemEnd = _this.readBuffer.indexOf('</item>')) >= 0) {
			...
			if(packet.data) {
				switch (packet.code) {
					case 'mper':
						if(!_this.metadata || !_this.metadata.id || (_this.metadata.id !== packet.data)) {
							_this.metadata = { id: packet.data };
						}
						break;
					case 'asal':
					...
					case 'assn':
						...
						break;
					case 'astm':
						_this.metadata.time = packet.data;
						break;
					case 'mden':
					...
					case 'PICT':
						...
						break;
					case 'prgr':
						const ps = packet.data.split('/').map((n) => parseInt(n));
						_this.emit('progress', (ps[1] - ps[0]) / (ps[2] - ps[0]));
						break;
					case 'caps':
						console.log('Player:', ShairportReader.playerState[packet.data]);
						_this.emit('state', ShairportReader.playerState[packet.data]);
						break;
				}
			}
		}
...
// Defaults to text
ShairportReader.decodeTypes = {
	none: ['PICT'],
	number: ['astm','mper','asdk','caps'],
	bool: []
};
ShairportReader.playerState = ['stop','play','pause'];
ShairportReader.doTheMario = /<item>\s*<type>([^<]+?)<\/type>\s*<code>([^<]+?)<\/code>\s*<length>([^<]+?)<\/length>(?:\s*<data(?:.+)>([^<]+?)<\/data>)?\s*<\/item>/;
ShairportReader.prototype.decodePacket = function(data){
	//console.log('raw',data);
	const match = ShairportReader.doTheMario.exec(data);
	if(match && match[1]) {
		...
		if(match[4] && ShairportReader.decodeTypes.number.indexOf(result.code) > -1) {
			result.data = parseInt(new Buffer(match[4], 'base64').toString('hex').replace(/^0+/, ''), 16);
		} else if(match[4] && ShairportReader.decodeTypes.bool.indexOf(result.code) > -1) {
			result.data = !!(new Buffer(match[4],'base64')[0]);
		} else if(match[4] && ShairportReader.decodeTypes.none.indexOf(result.code) > -1) {
			result.data = match[4];
		} else if(match[4]) {
